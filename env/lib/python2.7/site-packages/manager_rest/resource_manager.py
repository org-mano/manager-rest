...
class ResourceManager(object):
    ...
    def update_blueprint(self, blueprint_id, active, vendor):
        blueprint = self.sm.get(models.Blueprint, blueprint_id)
        #logging.info('update blueprint: id=' + blueprint_id)
        current_app.logger.info('========== update blueprint, id=' + blueprint_id)
        current_app.logger.info('active=' + active + ' vendor=' + vendor)
        current_app.logger.info(blueprint)
        blueprint.vendor = vendor
        if active.lower() == 'true':
            blueprint.active = True
        else:
            blueprint.active = False
        return self.sm.update(blueprint)

    def add_vim(self, param):
        current_app.logger.info('=============== add vim =============')
        new_vim = models.Vim(
            name='test_vim_name',
            type='openstack',
            status='ok',
            param='{"username": "demo", "password": "111111", "auth_url": "http://10.20.0.10:5000/v3"}'
        )
        return self.sm.put(new_vim)
    ...
    def create_vim(self, vim_name, type, inputs):
        new_vim = models.Vim(
            id=vim_name,
            name=vim_name,
            type=type,
    ...
    def delete_vim(self, vim_id):
        current_app.logger.info('delete_vim:  delete vim by id, vim_id=' + vim_id)
        vim = self.sm.get(models.Vim, vim_id)
        return self.sm.delete(vim)

    def create_vnf_pkg(self, vnf_pkg_id, img_url):
        current_app.logger.info('create vnf_pkg, pkg_id=' + vnf_pkg_id)
        vnf_pkg = models.Vnf_pkg(
            id = vnf_pkg_id,
            name = vnf_pkg_id,
            img_url = img_url,
            is_active = False,
    ...
    def delete_vnf_pkg(self, vnf_pkg_id):
        current_app.logger.info('delete_vnf_pkg:  delete vnf by id, pkg_id=' + vnf_pkg_id)
        vnf_pkg = self.sm.get(models.Vnf_pkg, vnf_pkg_id)
        return self.sm.delete(vnf_pkg)
        
    def create_vnf_img(self, vnf_img_id, vnf_pkg_id, vim_id):
        current_app.logger.info('create vnf_pkg, img_id=' + vnf_img_id)
        vnf_img = models.Vnf_img(
            id = vnf_img_id,
            name = vnf_img_id,
    ...
    def delete_vnf_img(self, vnf_img_id):
        current_app.logger.info('delete_vnf_img:  delete vnf by id, img_id=' + vnf_img_id)
        vnf_img = self.sm.get(models.Vnf_img, vnf_img_id)
        return self.sm.delete(vnf_img)
