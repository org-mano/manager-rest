...
logging.basicConfig(level=logging.DEBUG,
                ...
                datefmt='%H:%M:%S',
                filename='/var/log/cloudify/rest/myapp.log',
                filemode='a')
...
class BlueprintsIdUpdate(SecuredResource):

    @swagger.operation(
        #responseClass='List[{0}]'.format(models.Blueprint.__name__),
        nickname="updateById",
        notes="Update a blueprints by Id."
    )
    @exceptions_handled
    #@marshal_with(models.Blueprint)
    def put(self, blueprint_id, _include=None, **kwargs):
        """
        Update a blueprints
        """
        current_app.logging.info('put a blueprint by id, blueprint_id=' + blueprint_id)
        ...
        #active = request_dict.pop('active')
        #logging.info('active = ' + active)

        blueprint = get_resource_manager().update_blueprint(blueprint_id, active, vendor)
        return 'update ok: id=' + blueprint_id 
